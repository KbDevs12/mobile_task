...
import 'package:atlet_manager/widgets/gradient_button.dart'; // Import GradientButton
...
class _AddEditAtletScreenState extends State<AddEditAtletScreen> {
  ...
  String?
  _selectedCabangOlahragaNama; // To store selected CabangOlahraga display name

  final AtletService _atletService;
  final CabangOlahragaService _cabangOlahragaService =
      CabangOlahragaService(); // Instantiate CabangOlahragaService

  // Flag untuk menandai apakah sedang dalam proses loading.
  bool _isLoading = false;

  _AddEditAtletScreenState()
    : _atletService = AtletService(); // Initialize AtletService
  ...
  void initState() {
    ...
    _umurController = TextEditingController(
      text: widget.atlet?.umur.toString(),
    );
    _beratController = TextEditingController(
      text: widget.atlet?.beratBadan.toString(),
    );
    _tinggiController = TextEditingController(
      text: widget.atlet?.tinggiBadan.toString(),
    );
    _jenisKelamin = widget.atlet?.jenisKelamin;

    _selectedCabangOlahragaId =
        widget.atlet?.cabangOlahragaId; // Initialize for edit mode
    _selectedCabangOlahragaNama =
        widget.atlet?.cabangOlahragaNama; // Initialize for edit mode
  }
  ...
  void _saveAtlet() async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      if (_selectedCabangOlahragaId == null ||
          _selectedCabangOlahragaNama == null) {
        if (context.mounted)
          Notifikasi.show(
            context,
            'Pilih cabang olahraga untuk atlet ini.',
            isSuccess: false,
          );
        return;
      }
      ...
      try {
        final atletBaru = Atlet(
          ...
          cabangAtlet:
              _selectedCabangOlahragaNama!, // Use the formatted display name
        ...
        if (widget.atlet == null) {
          ...
          if (context.mounted)
            Notifikasi.show(context, 'Atlet berhasil ditambahkan.');
        } else {
          // Mode 'Edit'
          await _atletService.updateAtlet(atletBaru);
          if (context.mounted)
            Notifikasi.show(context, 'Data atlet berhasil diperbarui.');
        }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: GradientAppBar(
        title: widget.atlet == null
            ? 'Tambah Atlet'
            : 'Edit Atlet', // Dynamic title
      ),
              ...
              StreamBuilder<List<CabangOlahraga>>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (!snapshot.hasData || snapshot.data!.isEmpty) {
                    return const Text(
                      'Tidak ada cabang olahraga tersedia. Tambahkan cabang olahraga terlebih dahulu.',
                    );
                  }

                  final List<CabangOlahraga> cabangOlahragaList =
                      snapshot.data!;
                  return DropdownButtonFormField<String>(
                    ...
                    items: cabangOlahragaList.map<DropdownMenuItem<String>>((
                      CabangOlahraga cabangOlahraga,
                    ) {
                      return DropdownMenuItem<String>(
                        value: cabangOlahraga.id,
                        child: Text(
                          '${cabangOlahraga.namaCabang} - ${cabangOlahraga.pelatihNama}',
                        ), // Display format
                      );
                    }).toList(),
                    onChanged: (String? newValue) {
                      setState(() {
                        ...
                        final selectedCabang = cabangOlahragaList.firstWhere(
                          (c) => c.id == newValue,
                        );
                        _selectedCabangOlahragaNama =
                            '${selectedCabang.namaCabang} - ${selectedCabang.pelatihNama}';
                      });
                    },
                    validator: (value) =>
                        value == null ? 'Pilih cabang olahraga.' : null,
                  );
                },
              ),
              const SizedBox(height: 16),
