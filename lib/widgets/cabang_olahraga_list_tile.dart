            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  StreamBuilder<int>(
                    stream: atletService.getAtletCountByCabangOlahraga(
                      cabangOlahraga.id!,
                    ), // Get dynamic count
                    builder: (context, snapshot) {
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        return const Text(
                          'Jumlah Atlet: Menghitung...',
                  ...
                  onPressed: () async {
                    final confirm = await showDialog<bool>(
                      ...
                      builder: (context) => AlertDialog(
                        ...
                        content: Text(
                          'Apakah Anda yakin ingin menghapus cabang olahraga ${cabangOlahraga.namaCabang}?',
                        ),
                        actions: [
                          TextButton(
                    ...
                    if (confirm == true) {
                      try {
                        await cabangOlahragaService.deleteCabang(
                          cabangOlahraga.id!,
                        );
                        if (context.mounted) {
                          Notifikasi.show(
                            context,
                            'Cabang olahraga berhasil dihapus.',
                          );
                        }
                      } catch (e) {
                        if (context.mounted) {
